...
/**
 * G3b — recupera el perfil de un usuario que ya onboardeó al loguear en un
 * dispositivo nuevo, sin rehacer el onboarding. Hidrata el user store + a11y
 * desde el `UserOut` del login con política **gap-fill** (`deriveProfileHydration`
 * de @ynara/core, fuente de verdad compartida con mobile): solo rellena lo que
 * está vacío/default localmente, nunca pisa un cambio local. Imperativo
 * (getState/setState) porque corre en el `onSuccess` de la mutation, no en render.
 */
export function recoverProfileFromLogin(session: LoginResult): void {
  ...
  // Aplicar al <html> el a11y resultante (web-specific; espeja TuView/A11yStep).
  ...
  // Marcar el onboarding como completo (gate del routing en `(app)` layout). Se
  // usa `setState` directo en vez de `completeOnboarding()` a propósito: esa
  // acción stampea `onboardedAt: Date.now()` —sería la fecha del login en ESTE
  // dispositivo, no la del onboarding real, que el `UserOut` no trae—, así que se
  // deja `onboardedAt` como está (null) en vez de falsear el dato.
