 ...
 *
 * a11y: `<output>` con `aria-label` en el contenedor (output aporta role="status"
 * nativo, que implica aria-live="polite" + aria-atomic="true"). El orbe es
 * `aria-hidden` (decorativo). El lector anuncia la etiqueta del indicador una vez
 * al aparecer, sin repetir cada frame.
 *
...
export function TypingIndicator({ modeId }: Props) {
  return (
    <output aria-label="Ynara está escribiendo" className="flex items-center gap-2 px-1 py-2">
