 ...
 *
 * El formulario vive en `CheckinForm` y se remonta vía `key={open}`: así, al
 * cerrar y reabrir (el componente padre mantiene el Sheet montado), React
 * resetea mood/energía/nota a los defaults por nosotros, sin un useEffect que
 * observe la prop `open` y limpie el estado a mano (patrón recomendado por
 * react.dev: "resetting all state when a prop changes" → usar `key`).
 */
export function CheckinSheet({ open, onClose, onDone }: Props) {
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Check-in matinal" titleHidden>
      <CheckinForm key={open ? "open" : "closed"} onDone={handleDone} />
    </Sheet>
  );
}
