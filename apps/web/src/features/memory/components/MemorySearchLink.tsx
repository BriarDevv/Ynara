...
export function MemorySearchLink({ className }: { className?: string }) {
  return (
    <Link
      href="/buscar"
      className={cn(
        "flex min-h-[44px] items-center gap-3 rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-bg-soft)] px-4 py-2.5 text-left",
    ...
    >
      ...
      <span className="text-body text-[var(--color-ink-muted)]">Buscar en tu memoria…</span>
    </Link>
  );
}
