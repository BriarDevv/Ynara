...
| `20260625_1200_reminders_table.py` | `c3d4e5f6a7b8` | `b2c3d4e5f6a7` | **HEAD.** Tabla **operativa** `reminders` (recordatorios por-tiempo) + enum `reminder_status_enum` (pending/sent/cancelled) + FK a `users` (`ON DELETE CASCADE`) + índice en `user_id` + 2 compuestos: `(user_id, remind_at)` (listado por-usuario) y `(status, remind_at)` (scan GLOBAL del scheduler). El alta la hace el agente (`reminder.set`) o el POST REST; el scheduler (`dispatch_due_reminders`, beat 1 min) despacha los vencidos. `downgrade` dropea índices + tabla + el enum (round-trip limpio, simétrico). |
