...
| `20260622_1200_calendar_events_table.py` | `e5d9f2a73c1b` | `d1c2b3a49e87` | **HEAD.** Tabla **operativa** `calendar_events` (dominio Agenda, ADR-018) + enum `event_status_enum` + FK a `users` (`ON DELETE CASCADE`) + índices en `user_id` y `start_at`. NO toca `mode_enum` (ya existe; `mode` lo reusa con `create_type=False`). `downgrade` dropea índices + tabla + el enum (round-trip limpio). |
