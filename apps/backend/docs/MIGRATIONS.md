...
| `20260624_1400_index_tasks_user_id_scheduled_at.py` | `c1d2e3f4a5b6` | `b8e4f2a1c3d6` | Índice btree **aditivo** sobre la tabla **operativa** `tasks` (`(user_id, scheduled_at)`, ALB-04): el listado del dashboard "Hoy" filtra por `user_id` y ordena por `scheduled_at` → el compuesto acota el scan al user y sirve el orden sin sort. NO toca `ix_tasks_user_id` (FK). `downgrade` simétrico. |
| `20260625_1000_users_time_zone.py` | `a7b1c2d3e4f5` | `c1d2e3f4a5b6` | Agrega `time_zone` a `users` (tabla **sensible**, no sagrada): `VARCHAR(64)` NOT NULL `server_default='UTC'` (no rompe filas existentes, sin backfill). Lo usa el preámbulo de fecha/hora del chat + el "hoy" del dashboard en el huso del user. `downgrade` simétrico (dropea solo la columna). |
| `20260625_1100_device_tokens_table.py` | `b2c3d4e5f6a7` | `a7b1c2d3e4f5` | Tabla **operativa** `device_tokens` (tokens de push) + enum `device_platform_enum` (ios/android/web) + FK a `users` (`ON DELETE CASCADE`) + índice en `user_id` + UNIQUE sobre `token` (un dispositivo no se duplica; un re-registro re-asigna el dueño). `downgrade` dropea índice/unique + tabla + el enum (round-trip limpio). |
| `20260625_1200_reminders_table.py` | `c3d4e5f6a7b8` | `b2c3d4e5f6a7` | Tabla **operativa** `reminders` (recordatorios por-tiempo) + enum `reminder_status_enum` (pending/sent/cancelled) + FK a `users` (`ON DELETE CASCADE`) + índice en `user_id` + 2 compuestos: `(user_id, remind_at)` (listado por-usuario) y `(status, remind_at)` (scan GLOBAL del scheduler). El alta la hace el agente (`reminder.set`) o el POST REST; el scheduler (`dispatch_due_reminders`, beat 1 min) despacha los vencidos. `downgrade` dropea índices + tabla + el enum (round-trip limpio, simétrico). |
| `20260628_1200_users_preferences.py` | `e5f6a7b8c9d0` | `c3d4e5f6a7b8` | **HEAD.** Agrega `preferences` (JSONB) a `users` (tabla **sensible**, no sagrada): `server_default='{}'::jsonb` (no rompe filas existentes, sin backfill). Guarda las prefs **operativas** del onboarding (modos de interés + a11y) que persiste `POST /v1/onboarding` (ADR-026, fase G2); la memoria (mood/sobre-vos) es G4 aparte. `downgrade` simétrico (dropea solo la columna). |
