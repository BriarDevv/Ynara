...
async def chat(
    ...
    Orden transaccional (decision #2 M9): ``resolve_chat_session`` (flush, sin
    commit) -> ``route()`` -> ``session.commit()`` DESPUES de ``route()``. Es
    seguro commitear al final porque ``route()`` nunca propaga errores del LLM
    (overflow / error permanente devuelven fallback); asi se evita persistir una
    ``ChatSession`` huerfana y se mantiene el commit unico por request.
    ...
    #     asignado por el ORM; el commit es al final del request.
    ...
    # (5) Respuesta wire. actions defensivas (decision #4): se descartan las
    #     malformadas, no se filtra basura al cliente.
