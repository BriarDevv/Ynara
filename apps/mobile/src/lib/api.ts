...
// Mock-first encadenado por dominio: Memoria responde sync (Response | null); si
// el path no es de memoria, cae al mock de Hoy, que maneja sus paths o delega el
// resto —auth incluido— al fetch real. Con el flag off no se setea fetchImpl
// (queda el default = fetch global).
const mockFetch: (input: string, init?: RequestInit) => Promise<Response> = (input, init) => {
  const mem = memoryMockResponse(input, init);
  return mem ? Promise.resolve(mem) : todayMockFetch(input, init);
};
