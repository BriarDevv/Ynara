...
**Enfoque: mock-first con MSW** en las dos plataformas. ~~El backend todavía no expone `POST /v1/chat` por HTTP~~ — **actualización (2026-05-31): `POST /v1/chat` (sync JSON) y `POST /v1/chat/stream` (SSE) ya existen y usan auth** (M9 mergeado). Lo que falta del path real es el router completo con tool loop cablea las tools reales a producción (hoy stubs honestos) y el vLLM real (infra aparte, pendiente). `core/security.py` JWT está implementado y los endpoints `/v1/auth/register`, `/v1/auth/token`, `/v1/auth/me` están activos. El mock-first sigue siendo el enfoque recomendado mientras la infra vLLM no está en prod.
...
- `text: z.string().min(1).max(4000)` — se adelanta al `Field(max_length=4000)` que el backend cablea en M9 (hoy el Pydantic `ChatRequest.text` es `str` pelado; ver §9, decisión #6). Validar client-side no tiene contra.
...
- **Longitud máxima** del mensaje: **~4.000 chars** (decisión cerrada, fundada en el `max_model_len = 4096` de Gemma — el modelo más chico). El backend la **va a** enforçar (`max_length=4000` → 422) cuando se cablee en M9 (acción #3 de [`RESPUESTAS-CONTRATO-CHAT.md`](./RESPUESTAS-CONTRATO-CHAT.md); hoy `ChatRequest.text` es `str` pelado en el Pydantic). El frontend la valida antes de enviar, con contador al acercarse al límite — hasta M9 el límite vive solo en el cliente y en el mock.
...
**Auth es prerequisito del backend real, no del mock.** `core/security.py` JWT está implementado (#62) y M9 (`/v1/chat` sync + SSE, `/v1/sessions`, `/v1/memory`) está mergeado y cableado a auth. El mock no exige auth. Cuando se conecte al backend real, hace falta: (a) la inyección de `Authorization: Bearer <token>` en `lib/api.ts` de cada plataforma (hoy un TODO) y (b) que el vLLM real esté en infra. Sin el token, el chat real devuelve 401.

> ~~**Deuda detectada**: `AGENTS.md` decía que `core/security.py` está en `NotImplementedError`.~~ **Deuda saldada (2026-05-31)**: `core/security.py` JWT implementado (#62) y `AGENTS.md` + docs actualizados en el refresh de docs correspondiente.
