...
- **Latencia de lectura**: cada lectura cifrada agrega un descifrado in-memory. Para un turn típico (top-3 semantic + top-2 episodic = 5 records): ~5 operaciones AES-GCM, negligible en CPU moderno (<1ms total). *(Nota post-aceptación — SCAL-02, MITIGADO, no diferido: el costo del descifrado del top-K de las búsquedas y de los listados/export ya está optimizado e implementado, no es deuda pendiente. La key derivada por usuario se cachea —HKDF-SHA256 ya no se re-corre por record— vía `_derive_key_cached` (LRU por `(master_key, user_id)`) en `app/core/crypto.py`; `decrypt_many_for_user` deriva la key UNA vez y reusa una única instancia `AESGCM` para todo el lote; y el descifrado del lote se offloadea a un thread con `asyncio.to_thread` (`app/memory/semantic.py:114` / `episodic.py:108` en `search`; ídem en `list_all`), porque es CPU-bound y las primitivas de OpenSSL liberan el GIL — así no bloquea el event loop bajo concurrencia. El antiguo cuello de botella era re-derivar HKDF + reconstruir `AESGCM` por record en el top-K; eso ya no ocurre.)*
